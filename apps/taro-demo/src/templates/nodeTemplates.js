...
const shadow = (color, blur, offsetY, offsetX = 0, spread = 0) => ({
    color,
    blur,
    offsetX,
    offsetY,
...
const textShadow = (color, blur, offsetY, offsetX = 0) => ({
    color,
    blur,
    offsetX,
    offsetY,
...
export function createNodeTemplateDescriptor({ width, height, }) {
    return {
        type: "view",
        name: "Root",
        style: {
...
export function createNodeTemplateJSX(options) {
...
function featureRowDescriptor(title, subtitle, dotColor, bg) {
    return {
        type: "view",
        style: {
            flexDirection: "row",
...
function renderDescriptorChildren(children) {
    return children.map((child, index) => renderDescriptorNode(child, `node-${index}`));
}
function renderDescriptorNode(node, key) {
    const commonProps = {
        key,
        name: node.name,
        style: node.style,
    ...
    if (node.type === "scrollview") {
        ...
        return createElement(YogaScrollView, {
            ...commonProps,
